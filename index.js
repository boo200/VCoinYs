...
vCoinWS.onMissClickEvent(() => {
  ...
  if (++missCount > 10) {
    autobeep && beep()
    con('Плохое соединение с сервером, бот был остановлен.', true)
  }
})
...
vCoinWS.onGroupLoaded((groupInfo, groupData) => {
  if (groupData.name && groupInfo.place && groupInfo.score) {
    ...
    con('Место топа группы: ' + groupInfo.place)
...
rl.on('line', async (line) => {
  ...
  switch (line.trim().toLowerCase()) {
    ...
    case 'start':
    case 'run':
      if (vCoinWS.connected) return con('VCoinY уже работает!')
      xRestart = true
      startBooster()
      break
    case 'b':
    ...
    case 'sl':
    case 'setlimit':
      item = await rl.questionAsync('Введите новый лимит коинов / сек для SmartBuy & AutoBuy: ')
      limitCPS = parseInt(item.replace(/,/g, ''))
      con('Установлен новый лимит коинов / сек для SmartBuy & AutoBuy: ' + formatScore(limitCPS, true))
      break
    case 'to':
